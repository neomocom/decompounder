...
public abstract class CompoundWordTokenFilterBase extends TokenFilter {
    ...
    protected final boolean useGraphMode;
    ...
    protected CompoundWordTokenFilterBase(TokenStream input, CharArraySet dictionary) {
        this(input, dictionary, DEFAULT_MIN_WORD_SIZE, DEFAULT_MIN_SUBWORD_SIZE, DEFAULT_MAX_SUBWORD_SIZE,
                false, false);
    }
    ...
    protected CompoundWordTokenFilterBase(TokenStream input,
        ...
        this.useGraphMode = useGraphMode;
    ...
    public final boolean incrementToken() throws IOException {
        if (!tokens.isEmpty()) {
            assert current != null;
            boolean isFirstCompoundPart = tokens.size() > 1 && tokens.size() == posLengthAtt.getPositionLength();
            CompoundToken token = tokens.removeFirst();
            restoreState(current); // keep all other attributes untouched
            termAtt.setEmpty().append(token.txt);
            offsetAtt.setOffset(token.startOffset, token.endOffset);
            if (useGraphMode) {
                posIncAtt.setPositionIncrement(isFirstCompoundPart ? 0 : 1);
        ...
        if (input.incrementToken()) {
            ...
            if (termAtt.length() >= this.minWordSize) {
                ...
                if (!tokens.isEmpty()) {
                    if (useGraphMode) {
                        posLengthAtt.setPositionLength(tokens.size());
                    }
